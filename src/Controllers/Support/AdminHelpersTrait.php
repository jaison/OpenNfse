...
trait AdminHelpersTrait
{
    ...
    public function getConfigFieldHelp(string $name): string
    {
        $help = [
            ...
            'auto_emit_on_payment' => 'Quando ativo, habilita a automação de emissão (no pagamento e, se configurado, também na criação da fatura para Unpaid).',
            ...
            'allow_unpaid_manual_emit' => 'Quando ativo, permite emissão manual no admin mesmo com fatura Unpaid.',
            'allow_unpaid_auto_emit' => 'Quando ativo, enfileira NFS-e automaticamente na criação da fatura mesmo Unpaid (útil quando o cliente só paga após receber a nota). Continua respeitando o custom field do cliente e a fila/cron.',
