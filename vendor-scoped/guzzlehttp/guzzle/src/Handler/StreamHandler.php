...
namespace OpenNfseVendor\GuzzleHttp\Handler;

use OpenNfseVendor\GuzzleHttp\Exception\ConnectException;
use OpenNfseVendor\GuzzleHttp\Exception\RequestException;
use OpenNfseVendor\GuzzleHttp\Promise as P;
use OpenNfseVendor\GuzzleHttp\Promise\FulfilledPromise;
use OpenNfseVendor\GuzzleHttp\Promise\PromiseInterface;
use OpenNfseVendor\GuzzleHttp\Psr7;
use OpenNfseVendor\GuzzleHttp\TransferStats;
use OpenNfseVendor\GuzzleHttp\TransportSharing;
use OpenNfseVendor\GuzzleHttp\Utils;
...
class StreamHandler
{
    ...
    private static function triggerUnsupportedRequestOptionDeprecations(RequestInterface $request, array $options): void
    {
        if (\array_key_exists('curl', $options) && $options['curl'] !== null && $options['curl'] !== [] && !self::isCurlOptionGeneratedByAuth($options)) {
            \OpenNfseVendor\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing the "curl" request option to the stream handler is deprecated; guzzlehttp/guzzle 8.0 will reject this option because the stream handler ignores cURL options.');
        }
        if (self::usesDigestAuth($options)) {
            \OpenNfseVendor\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing digest authentication to the stream handler is deprecated; guzzlehttp/guzzle 8.0 will reject digest authentication with the stream handler because it is only supported by cURL handlers.');
        }
        if (\array_key_exists('expect', $options) && $options['expect'] !== \false && $request->hasHeader('Expect')) {
            \OpenNfseVendor\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing the "expect" request option to the stream handler is deprecated when it adds an Expect header; guzzlehttp/guzzle 8.0 will reject this option because the stream handler does not support Expect: 100-Continue.');
        }
    }
