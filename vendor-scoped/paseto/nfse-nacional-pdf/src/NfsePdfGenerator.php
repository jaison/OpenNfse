...
class NfsePdfGenerator
{
    ...
    private $headerInfo = ['municipalityLine' => null, 'secretariatLine' => null, 'phoneLine' => null, 'emailLine' => null];
    public function __construct()
    {
        $this->pdf = new TCPDF('P', 'mm', 'A4', \true, 'UTF-8', \false);
        ...
        $this->pdf->SetAutoPageBreak(\true, $this->margin);
    ...
    public function parseXml($xmlFile)
    {
        ...
        $this->data = ['chaveAcesso' => $chaveAcesso, 'numeroNfse' => (string) $infNFSe->nNFSe, 'localEmissao' => (string) $infNFSe->xLocEmi, 'localPrestacao' => (string) $infNFSe->xLocPrestacao, 'localIncidencia' => (string) $infNFSe->xLocIncid, 'tribNac' => (string) $infNFSe->xTribNac, 'dataProcessamento' => $this->formatDateTime((string) $infNFSe->dhProc), 'numeroDFSe' => (string) $infNFSe->nDFSe, 'emitente' => ['cnpj' => $this->formatCnpjCpf((string) $infNFSe->emit->CNPJ), 'nome' => (string) $infNFSe->emit->xNome, 'IM' => (string) ($infNFSe->emit->IM ?? ''), 'logradouro' => (string) $infNFSe->emit->enderNac->xLgr, 'numero' => (string) $infNFSe->emit->enderNac->nro, 'bairro' => (string) $infNFSe->emit->enderNac->xBairro, 'municipio' => (string) $infNFSe->emit->enderNac->cMun, 'uf' => (string) $infNFSe->emit->enderNac->UF, 'cep' => $this->formatCep((string) $infNFSe->emit->enderNac->CEP), 'fone' => $this->formatPhone((string) $infNFSe->emit->fone), 'email' => (string) $infNFSe->emit->email], 'tomador' => ['cnpj' => $this->formatCnpjCpf((string) ((($dps->toma->CNPJ ?? $dps->toma->CPF) ?? $dps->toma->NIF) ?? '-')), 'nome' => (string) $dps->toma->xNome, 'IM' => (string) ($dps->toma->IM ?? ''), 'email' => (string) $dps->toma->email, 'logradouro' => (string) $dps->toma->end->xLgr, 'numero' => (string) $dps->toma->end->nro, 'complemento' => (string) $dps->toma->end->xCpl, 'bairro' => (string) $dps->toma->end->xBairro, 'municipio' => (string) (($dps->toma->end->endNac->cMun ?? $dps->toma->end->endExt->cMun) ?? ''), 'uf' => (string) $infNFSe->emit->enderNac->UF, 'cep' => $this->formatCep((string) (($dps->toma->end->endNac->CEP ?? $dps->toma->end->endExt->CEP) ?? '')), 'fone' => $this->formatPhone((string) $dps->toma->fone)], 'servico' => ['codTribNac' => (string) $dps->serv->cServ->cTribNac, 'descricao' => (string) $dps->serv->cServ->xDescServ], 'valores' => ['valorServico' => (float) $dps->valores->vServPrest->vServ, 'valorLiquido' => (float) $infNFSe->valores->vLiq, 'valorTotalRet' => (float) $infNFSe->valores->vTotalRet, 'bcIssqn' => (float) ($infNFSe->valores->vBC ?? 0), 'aliqAplicada' => (float) ($infNFSe->valores->pAliqAplic ?? 0), 'issqnApurado' => (float) ($infNFSe->valores->vISSQN ?? 0)], 'dps' => ['numero' => (string) $dps->nDPS, 'serie' => (string) $dps->serie, 'competencia' => $this->formatDate((string) $dps->dCompet), 'dataEmissao' => $this->formatDateTime((string) $dps->dhEmi)], 'tributacao' => ['tribISSQN' => (string) $dps->valores->trib->tribMun->tribISSQN, 'tpRetISSQN' => (string) $dps->valores->trib->tribMun->tpRetISSQN, 'totTribFed' => (float) $dps->valores->trib->totTrib->pTotTrib->pTotTribFed, 'totTribEst' => (float) $dps->valores->trib->totTrib->pTotTrib->pTotTribEst, 'totTribMun' => (float) $dps->valores->trib->totTrib->pTotTrib->pTotTribMun, 'opSimpNac' => $opSimpNac, 'regApTribSN' => $regApTribSN]];
        return $this;
    }
    ...
    private function drawDocumentBorder()
    {
        ...
        // Total width minus both margins
        $height = $pageHeight - (2 * $this->margin - 5);
        // Total height minus both margins
    ...
    private function addHorizontalLine($addLineHeight = \true)
    {
        $y = $this->pdf->GetY();
        $pageWidth = 210;
        // A4 width in mm
    ...
    private function addHeader()
    {
        ...
        $centerX = 62;
        // Adjusted for 2mm margin (was 70, now 70-8=62)
        ...
        $rightX = 137;
        // base X for the right-side header block
        $blockWidth = 55;
        // total width originally used for the right header cell
        $logoWidth = 12;
        $gap = 2;
        ...
        if (!empty($this->logoSvg)) {
            ...
            $logoX = $rightX;
            // left edge of the right header block
    ...
    private function addDadosNfse()
    {
        ...
        $col1X = $this->margin;
        // Adjusted for 2mm margin (was 10)
        $col2X = 47;
        // Adjusted for 2mm margin (was 55, now 55-8=47)
        $col3X = 97;
        // Adjusted for 2mm margin (was 105, now 105-8=97)
        $col4X = 147;
        // Adjusted for 2mm margin (was 155, now 155-8=147)
        $col1W = 45;
        $col2W = 50;
        $col3W = 50;
        $col4W = 45;
    ...
    private function addEmitente()
    {
        ...
        $this->pdf->SetFont('helvetica', '', 8);
        $opSimpNacMap = ['1' => 'Não Optante', '2' => 'Optante - Microempreendedor Individual (MEI)', '3' => 'Optante - Microempresa ou Empresa de Pequeno Porte (ME/EPP)'];
        $regApTribSNMap = [1 => 'Regime de apuração dos tributos federais e municipal pelo SN', 2 => 'Regime de apuração dos tributos federais pelo SN e o ISSQN pela NFS-e conforme respectiva legislação municipal do tributo', 3 => 'Regime de apuração dos tributos federais e municipal pela NFS-e conforme respectivas legilações federal e municipal de cada tributo'];
    ...
    private function addServico()
    {
        ...
        $codTrib = $this->truncateTextToLines($codTribFormatted . ' - ' . $this->data['tribNac'], $col1W, 3);
        $dataRowY = $startY + 4;
        $codTribHeight = $this->pdf->getStringHeight($col1W, $codTrib);
        $this->pdf->SetXY($col1X, $dataRowY);
        $this->pdf->MultiCell($col1W, 4, $codTrib, 0, 'L', \false, 0, $col1X, $dataRowY, \true, 0, \false, \true, 0, 'T', \false);
        ...
        $this->pdf->SetXY($col2X, $row2Y);
        $this->pdf->MultiCell($col2W + $col3W + $col4W, 4, $descricao, 0, 'L', \false, 1, $col2X, $row2Y, \true, 0, \false, \true, 0, 'T', \false);
    ...
    private function addTributacao()
    {
        ...
        $tpRetISSQNMap = ['1' => 'Não Retido', '2' => 'Retido pelo Tomador', '3' => 'Retido pelo Intermediário'];
    ...
    private function addValores()
    {
        ...
        $issqnRetido = in_array($tpRetISSQN, ['2', '3'], \true) && $val['issqnApurado'] > 0 ? 'R$ ' . number_format($val['issqnApurado'], 2, ',', '.') : '-';
        $startY = $this->pdf->GetY();
        // Header row
        $this->pdf->SetFont('helvetica', 'B', 7);
        $this->pdf->SetXY($col1X, $startY);
    ...
    private function addTotaisTributos()
    {
        ...
        // Adjusted for 2mm margin (was 70, now 70-8=62)
        $col3X = 122;
        // Adjusted for 2mm margin (was 130, now 130-8=122)
        $col1W = 60;
        $col2W = 60;
        $col3W = 60;
        $this->pdf->SetFont('helvetica', 'B', 7);
