...
use DOMNode;
use Exception;
class XmlSigner implements \Nfse\Signer\SignerInterface
{
    private const CANONICAL = [\true, \false, null, null];
    public function __construct(private \Nfse\Signer\Certificate $certificate)
    {
    }
    /**
     * Sign XML content
     ...
     */
    public function sign(string $content, string $tagname, string $mark = 'Id', int $algorithm = \OPENSSL_ALGO_SHA1, array $canonical = self::CANONICAL, string $rootname = '', array $options = []): string
    {
        if (empty($content)) {
        ...
        $this->createSignature($dom, $root, $node, $mark, $algorithm, $canonical);
        $xml = $dom->saveXML();
        $xml = str_replace(["\n", "\r", "\t"], '', $xml);
        return $xml;
    }
    private function createSignature(DOMDocument $dom, DOMNode $root, DOMElement $node, string $mark, int $algorithm, array $canonical): void
    {
    ...
    private function makeDigest(DOMNode $node, string $algorithm, array $canonical): string
    {
        ...
        $hashValue = hash($algorithm, $c14n, \true);
        return base64_encode($hashValue);
    }
    private function canonize(DOMNode $node, array $canonical): string
    {
        return $node->C14N($canonical[0] ?? \true, $canonical[1] ?? \false, $canonical[2] ?? null, $canonical[3] ?? null);
    }
}
