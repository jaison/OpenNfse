...
use OpenNfseVendor\GuzzleHttp\Client;
use OpenNfseVendor\GuzzleHttp\Exception\GuzzleException;
use OpenNfseVendor\GuzzleHttp\RequestOptions;
...
class SefinClient implements SefinNacionalInterface
{
    ...
    {
        return new Client(['base_uri' => rtrim($this->baseUrl, '/') . '/', 'curl' => [\CURLOPT_SSLCERTTYPE => 'P12', \CURLOPT_SSLCERT => $this->resolveCertificatePath(), \CURLOPT_SSLCERTPASSWD => $this->context->certificatePassword, \CURLOPT_IPRESOLVE => \CURL_IPRESOLVE_V4, \CURLOPT_CONNECTTIMEOUT => 30, \CURLOPT_TIMEOUT => 60, \CURLOPT_HTTP_VERSION => \CURL_HTTP_VERSION_1_1, \CURLOPT_SSL_VERIFYHOST => 0, \CURLOPT_SSL_VERIFYPEER => 0], RequestOptions::HEADERS => ['Content-Type' => 'application/json', 'Accept' => 'application/json']]);
    }
    private function post(string $endpoint, array $data): array
    {
        try {
            $response = $this->httpClient->post($endpoint, [RequestOptions::JSON => $data]);
    ...
    public function emitirNfse(string $dpsXmlGZipB64): EmissaoNfseResponse
    {
        ...
        return new EmissaoNfseResponse(tipoAmbiente: $response['tipoAmbiente'] ?? null, versaoAplicativo: $response['versaoAplicativo'] ?? null, dataHoraProcessamento: $response['dataHoraProcessamento'] ?? null, idDps: $response['idDps'] ?? null, chaveAcesso: $response['chaveAcesso'] ?? null, nfseXmlGZipB64: $response['nfseXmlGZipB64'] ?? null, alertas: $this->mapMensagens($response['alertas'] ?? []), erros: $this->mapMensagens($response['erros'] ?? []));
    }
    public function consultarNfse(string $chaveAcesso): ConsultaNfseResponse
    {
        $response = $this->get("nfse/{$chaveAcesso}");
        return new ConsultaNfseResponse(tipoAmbiente: $response['tipoAmbiente'] ?? null, versaoAplicativo: $response['versaoAplicativo'] ?? null, dataHoraProcessamento: $response['dataHoraProcessamento'] ?? null, chaveAcesso: $response['chaveAcesso'] ?? null, nfseXmlGZipB64: $response['nfseXmlGZipB64'] ?? null);
    }
    public function consultarDps(string $idDps): ConsultaDpsResponse
    {
        $response = $this->get("dps/{$idDps}");
        return new ConsultaDpsResponse(tipoAmbiente: $response['tipoAmbiente'] ?? null, versaoAplicativo: $response['versaoAplicativo'] ?? null, dataHoraProcessamento: $response['dataHoraProcessamento'] ?? null, idDps: $response['idDps'] ?? null, chaveAcesso: $response['chaveAcesso'] ?? null);
    }
    public function registrarEvento(string $chaveAcesso, string $eventoXmlGZipB64): RegistroEventoResponse
    {
        $response = $this->post("nfse/{$chaveAcesso}/eventos", ['pedidoRegistroEventoXmlGZipB64' => $eventoXmlGZipB64]);
        return new RegistroEventoResponse(tipoAmbiente: $response['tipoAmbiente'] ?? null, versaoAplicativo: $response['versaoAplicativo'] ?? null, dataHoraProcessamento: $response['dataHoraProcessamento'] ?? null, eventoXmlGZipB64: $response['eventoXmlGZipB64'] ?? null);
    }
    ...
    {
        return array_map(fn($m) => new MensagemProcessamentoDto(mensagem: $m['mensagem'] ?? $m['Mensagem'] ?? null, parametros: $m['parametros'] ?? $m['Parametros'] ?? null, codigo: $m['codigo'] ?? $m['Codigo'] ?? null, descricao: $m['descricao'] ?? $m['Descricao'] ?? null, complemento: $m['complemento'] ?? $m['Complemento'] ?? null), $mensagens);
    }
    /**
