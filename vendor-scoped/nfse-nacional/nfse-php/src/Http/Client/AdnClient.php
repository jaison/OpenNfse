...
class AdnClient implements AdnDanfseInterface
{
    ...
    {
        $baseUrl = $this->context->ambiente === TipoAmbiente::Producao ? self::URL_PRODUCTION : self::URL_HOMOLOGATION;
        return new Client(['base_uri' => $baseUrl, 'curl' => [\CURLOPT_SSLCERTTYPE => 'P12', \CURLOPT_SSLCERT => $this->resolveCertificatePath(), \CURLOPT_SSLCERTPASSWD => $this->context->certificatePassword, \CURLOPT_IPRESOLVE => \CURL_IPRESOLVE_V4, \CURLOPT_CONNECTTIMEOUT => 30, \CURLOPT_TIMEOUT => 60, \CURLOPT_HTTP_VERSION => \CURL_HTTP_VERSION_1_1, \CURLOPT_SSL_VERIFYHOST => 0, \CURLOPT_SSL_VERIFYPEER => 0], RequestOptions::HEADERS => ['Content-Type' => 'application/json', 'Accept' => 'application/json']]);
    }
    private function get(string $endpoint): array
    {
        try {
            ...
            return $decoded;
        } catch (\OpenNfseVendor\GuzzleHttp\Exception\RequestException $e) {
     ...
     * ADN Contribuinte
     */
    public function baixarDfeContribuinte(int $nsu, ?string $cnpjConsulta = null, bool $lote = \true): DistribuicaoDfeResponse
    {
        $queryParams = [];
        if ($cnpjConsulta) {
     ...
     * ADN Município
     */
    public function baixarDfeMunicipio(int $nsu, ?TipoNsu $tipoNSU = null, bool $lote = \true): DistribuicaoDfeResponse
    {
        $queryParams = [];
        if ($tipoNSU) {
    ...
    {
        try {
            $response = $this->httpClient->post('/adn/DFe', [RequestOptions::JSON => ['LoteXmlGZipB64' => [$xmlZipB64]]]);
    ...
    public function consultarParametrosConvenio(string $codigoMunicipio): ResultadoConsultaConfiguracoesConvenioResponse
    {
        ...
        return new ResultadoConsultaConfiguracoesConvenioResponse(['mensagem' => $response['mensagem'] ?? null, 'parametrosConvenio' => isset($response['parametrosConvenio']) ? new ParametrosConfiguracaoConvenioDto($response['parametrosConvenio']) : null]);
    }
    ...
    {
        $listaNsu = array_map(fn($item) => new DistribuicaoNsuDto(['NSU' => $item['NSU'] ?? null, 'chAcesso' => $item['ChaveAcesso'] ?? null, 'dfeXmlGZipB64' => $item['ArquivoXml'] ?? null]), $response['LoteDFe'] ?? []);
        ...
        return new DistribuicaoDfeResponse(['tpAmb' => $response['TipoAmbiente'] ?? null, 'verAplic' => $response['VersaoAplicativo'] ?? null, 'dhProc' => $response['DataHoraProcessamento'] ?? null, 'ultNSU' => $ultimoNsu, 'maiorNSU' => $maiorNsu, 'alertas' => $this->mapMensagens($response['Alertas'] ?? []), 'erros' => $this->mapMensagens($response['Erros'] ?? []), 'lNSU' => $listaNsu]);
    }
    private function mapAliquotaResponse(array $response): ResultadoConsultaAliquotasResponse
    {
        ...
        return new ResultadoConsultaAliquotasResponse(['mensagem' => $response['mensagem'] ?? null, 'aliquotas' => $aliquotas]);
    }
     ...
     */
    private function handleException(\OpenNfseVendor\GuzzleHttp\Exception\RequestException|\Exception $e)
    {
        $responseBody = '';
        if ($e->hasResponse()) {
