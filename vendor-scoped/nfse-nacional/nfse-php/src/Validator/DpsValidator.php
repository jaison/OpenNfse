...
class DpsValidator
{
    public function validate(DpsData $dps): \Nfse\Validator\ValidationResult
    {
        $errors = [];
        $infDps = $dps->infDps;
        if ($infDps === null) {
            return \Nfse\Validator\ValidationResult::failure(['InfDpsData is required.']);
        }
        ...
        if (count($errors) > 0) {
            return \Nfse\Validator\ValidationResult::failure($errors);
        }
        return \Nfse\Validator\ValidationResult::success();
    }
    ...
    private function validateTomador(InfDpsData $infDps, array &$errors): void
    {
        ...
        if ($isIdentified) {
            ...
            } else if ($tomador->endereco->codigoMunicipio === null) {
    ...
    private function validateServico(InfDpsData $infDps, array &$errors): void
    {
        ...
        $constructionCodes = ['070201', '070202', '070401', '070501', '070502', '070601', '070602', '070701', '070801', '071701', '071901'];
