...
abstract class DataTransferObject
{
    ...
    public function __construct(...$args)
    {
        ...
        foreach ($class->getProperties() as $property) {
            $property->setValue(\Spatie\DataTransferObject\Arr::get($args, $property->name, $property->getDefaultValue()));
            $args = \Spatie\DataTransferObject\Arr::forget($args, $property->name);
        }
    ...
    {
        return array_map(fn(mixed $parameters) => new static($parameters), $arrayOfParameters);
    }
    ...
    public function toArray(): array
    {
        if (count($this->onlyKeys)) {
            $array = \Spatie\DataTransferObject\Arr::only($this->all(), $this->onlyKeys);
        } else {
            $array = \Spatie\DataTransferObject\Arr::except($this->all(), $this->exceptKeys);
        }
    ...
    protected function parseArray(array $array): array
    {
        foreach ($array as $key => $value) {
            if ($value instanceof \Spatie\DataTransferObject\DataTransferObject) {
