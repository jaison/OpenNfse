...
final class NfseXmlExtractor
{
    ...
    public static function extractChaveAcesso(string $xml): ?string
    {
        return self::withDocument($xml, static function (\DOMXPath $xp): ?string {
            foreach (['chNFSe', 'chAcesso', 'chaveAcesso', 'ChaveAcesso'] as $nodeName) {
    ...
    {
        return self::withDocument($xml, static function (\DOMXPath $xp): ?string {
            foreach (['tpEvento', 'tipoEvento', 'TipoEvento'] as $nodeName) {
                $nodes = $xp->query('//*[local-name()="' . $nodeName . '"]');
                if ($nodes && $nodes->length > 0) {
                    $value = preg_replace('/\D+/', '', trim((string) $nodes->item(0)?->textContent));
                    if ($value !== '') {
                        return $value;
                    }
                }
    ...
    public static function extractEventDate(string $xml): ?string
    {
        return self::withDocument($xml, static function (\DOMXPath $xp): ?string {
            foreach (['dhEvento', 'dataHoraEvento', 'DataHoraEvento'] as $nodeName) {
                $nodes = $xp->query('//*[local-name()="' . $nodeName . '"]');
                if ($nodes && $nodes->length > 0) {
                    $raw = trim((string) $nodes->item(0)?->textContent);
                    if ($raw === '') {
