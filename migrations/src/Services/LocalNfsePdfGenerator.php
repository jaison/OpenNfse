...
final class LocalNfsePdfGenerator
{
    private $pdf;
    private $data;
    private $margin = 5;
    ...
    public function parseXml($xmlFile)
    {
        ...
        $tomadorUf = (string) $tomadorEndNac->UF;
        if ($tomadorUf === '') {
            $tomadorUf = (string) $tomadorEnd->UF;
        }
    ...
    private function addHeader()
    {
        ...
        $logoPath = dirname(__DIR__, 2) . '/vendor-scoped/paseto/nfse-nacional-pdf/assets/logo-nfse-assinatura-horizontal.png';
    ...
    private function addTomador()
    {
        ...
        if ($mun === (string) preg_replace('/\D/', '', (string) ($toma['municipio'] ?? ''))) {
            $fallbackMunicipio = trim((string) ($this->data['localIncidencia'] ?? ''));
            $fallbackUf = trim((string) ($toma['uf'] ?? ''));
            if ($fallbackMunicipio !== '') {
                $mun = $fallbackMunicipio . ($fallbackUf !== '' ? ' - ' . $fallbackUf : '');
            }
        }
    ...
    private function resolveMunicipioDisplay(string $ibgeCode): string
    {
        ...
        try {
            (new IbgeService())->ensureMunicipiosCatalogPopulated();
        } catch (\Throwable $e) {
        }
        ...
        $apiDisplay = $this->resolveMunicipioDisplayFromApi($ibgeCode);
        if ($apiDisplay !== null) {
            return $apiDisplay;
        }
    ...
    private function resolveMunicipioDisplayFromApi(string $ibgeCode): ?string
    {
        $response = $this->fetchUrl('https://servicodados.ibge.gov.br/api/v1/localidades/municipios/' . rawurlencode($ibgeCode));
        if ($response === null) {
            return null;
        }
        ...
        $uf = trim((string) ($data->microrregiao->mesorregiao->UF->sigla ?? ''));
        if ($nome === '') {
            return null;
        }
        ...
        if (class_exists(\WHMCS\Database\Capsule::class)) {
            try {
                (new IbgeMunicipioRepository())->upsert(
                    $ibgeCode,
                    NameNormalizer::normalize($nome),
                    $nome,
                    $uf
                );
            } catch (\Throwable $e) {
